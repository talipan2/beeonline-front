	...
	>
		<template #header>
			<UiBreadCrumb
				:list="[
					{ label: 'Главная', link: '/' },
					{ label: 'Каталог производителей', link: '' },
				]"
			/>
		</template>
		<template #content>
			...
			>
				...
				>
					<div
						...
						:class="
							isMobile
								? 'new-service__filter_type_mobile'
...
<script setup>
	...
	const handleUpdateFilter = (data) => {
		...
		if (!data || data.length === 0) {
			...
			loading.value = true;
			organizationStore
				.getPubCardsList({ type: 'performer' })
				.then((res) => {
		...
		const newQuery = {
			...
			free_samples:
				data.free_samples && data.free_samples.length
					? data.free_samples.join(',')
					: undefined,
			free_stock: data.free_stock != null ? data.free_stock : undefined,
			materials_own:
				data.material && data.material.length && data.material.includes(0)
					? 1
					: undefined,
			materials_tolling:
				data.material && data.material.length && data.material.includes(1)
					? 1
					: undefined,
		...
		const filterQuery = {
			...
			free_samples:
				data.free_samples && data.free_samples.length
					...
					: undefined,
			free_stock:
				data.free_stock != null ? Boolean(data.free_stock) : undefined,
			materials_own:
				data.material && data.material.length && data.material.includes(0)
					? 1
					: undefined,
	...
	const updateServiceCardRef = (newVal) => {
		...
		if (newVal) {
			tutorialRefs.value = [
				...tutorialRefs.value,
				{
					component: serviceCardRef.value,
					content: 'Нажмите, чтобы просмотреть услугу',
				},
			];
		}
	};
	...
	onMounted(() => {
		...
		if (Object.keys(router.currentRoute.value.query).length > 0) {
			...
			params = {
				...
				countries: [
					query.countries &&
						query.countries.split(',').map((item) => Number(item)),
				].flat(),
				regions: [
					query.regions && query.regions.split(',').map((item) => Number(item)),
				].flat(),
				is_stm: query.is_stm ? Number(query.is_stm) : undefined,
				free_samples: query.free_samples
					? query.free_samples.split(',').map((item) => Number(item))
					: undefined,
				free_stock: query.free_stock ? Number(query.free_stock) : undefined,
				materials_own: query.materials_own
					? Number(query.materials_own)
					: undefined,
				materials_tolling: query.materials_tolling
			...
			filter.value = {
				...
				location: {
					...
					regions: query.regions
						? query.regions.split(',').map((item) => Number(item))
						: [],
				},
				is_stm: query.is_stm ? Number(query.is_stm) : undefined,
				free_samples: query.free_samples
		...
		organizationStore
			.getPubCardsList({ ...params, type: 'performer' })
			.then((res) => {
				if (res) {
					servicesList.value = res.data;
					page.value = {
