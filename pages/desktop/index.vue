<template>
  <NuxtLayout name="profile" :title="`Кабинет ${translateRole}`" class="desktop-layout">
    <template #header>
      <UiBreadCrumb :list="[{label: 'Главная', link: '/'}, { label: `Кабинет ${translateRole}`, link: '' }]" />
    </template>
    <template #content>
      <Desktop :getEntity="getEntity" :role="role" :filterList="filterList"/>
    </template>
  </NuxtLayout>
</template>
