...
export const useEntityStore = defineStore('entity', {
	...
	actions: {
		...
		async prepareRaiseService(id) {
			return await useApi().get(`/pubcards/performer/${id}/raise`);
		},
		async raiseService(id) {
			return await useApi().post(`/pubcards/performer/${id}/raise`);
		},

		async prepareOrderIntro(id) {
			return await useApi().get(`/orders/${id}/intro`);
		},
		async setOrderIntro(id, data, form) {
			return await useApi().post(`/orders/${id}/intro`, data, form);
		},
		...
		async getSelfServices(organizationId, params) {
			try {
				const response = await serviceApi.getSelfServices(
					organizationId,
					params
				);
		...
		async getBatches() {
			try {
				...
				if (response.data) {
					...
					this.entityData.serviceBatch = response.data.map((item) => ({
						id: item?.id,
						label: item?.name,
						value: item?.id,
					}));
