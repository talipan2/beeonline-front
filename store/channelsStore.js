...
export const useChannelsStore = defineStore("channelsStore", {
    ...
    getters: {
        userChannel(state) {
			if (!state._userChannel) {
				state._userChannel = this.private(`user.${useUserStore().userData?.id}`);
			}
			return state._userChannel;
		},
		orgChannel(state) {
			if (!state._orgChannel) {
				state._orgChannel = this.private(`organization.${useUserStore().userOrganizationId}`);
			}
    ...
    actions: {
		initEcho() {
			...
			this.laravelEcho.connector.pusher.connection.bind('connected', () => {
		...
		private(channel) {
			...
			return this.laravelEcho.private(channel)
			.subscribed(() => {
