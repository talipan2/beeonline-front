...
export default {
	...
	async uploadTzFiles(id, data) {
		return axios.post(`/orders/${id}/attach-tz-files`, {
			media_ids: [...data],
		});
	},

	async updateOrderStep(id, currentStep) {
		return axios.patch(`orders/${id}/update-step`, {
	...
	async orderUnpublish(id, values, form) {
		return useApi().post(`orders/${id}/unpublish`, values, form);
	},

	// Получение исполнителей, с которыми были сообщения в чате
	async getOrderPublicationsPerformers(id, params) {
		return useApi().get(`order-publications/${id}/performers`, params);
	},
};
