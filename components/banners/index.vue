...
<script setup>
	...
	const currentImageUrl = computed(() => {
		...
		if (isMobile.value) {
			return (
				props.banner.images.sm ||
				props.banner.images.md ||
				props.banner.images.lg ||
				''
			);
		}
		if (isTablet.value) {
	...
	const showBanner = computed(() => {
		...
		const now = new Date();
		if (
			props.banner.date_active_from &&
			new Date(props.banner.date_active_from) > now
		)
			return false;
		if (
			props.banner.date_active_to &&
			new Date(props.banner.date_active_to) < now
		)
			return false;
