<template>
	<div class="gallery-load-secondary">
		<template v-if="isPreview">
			<div
				class="gallery-load-secondary__wrapper gallery-load-secondary__wrapper_type_preview"
			>
				<template v-if="localModelValue?.length > 0">
					<div
						class="gallery-load-secondary__item-container"
						v-for="element in localModelValue"
						:key="element.id"
					>
						<div
							class="gallery-load-secondary__item"
							:key="element.id"
						>
							<a
								:href="element.url"
			...
			<div class="gallery-load-secondary__slider-wrapper">
				<template v-if="localModelValue?.length > 0">
					...
					>
						<SwiperSlide
							v-for="(image, index) in localModelValue"
							:key="index"
							class="gallery-load-secondary__slide"
						>
							<a
								:href="image.url"
								class=""
					...
					<div
						class="gallery-load-secondary__slider-navigation"
						v-if="localModelValue.length > 1"
					>
						<UiButton
							type="button"
			...
			>
				<template #header>
					<div class="gallery-load-secondary__container">
						<div
							class="gallery-load-secondary__loader"
							:class="{
								'gallery-load-secondary__loader_disabled': isUploadDisabled,
							}"
						>
							<input
								type="file"
							...
							<div class="gallery-load-secondary__placeholder">
								<SvgoCamera
									class="svg-lx gallery-load-secondary__camera"
									fill="#6937a5"
								/>
								<span
									class="gallery-load-secondary__title"
									v-if="label"
								>
									{{ label }}
								</span>
								<CommonProgressBar
									class="gallery-load-secondary__progress"
									:progress="progress"
				...
				<template #item="{ element }">
					<div class="gallery-load-secondary__item-container">
						<div
							class="gallery-load-secondary__item gallery-load-secondary__item_type_drag"
							:key="element.id"
						>
							<UiImage
								:src="element.croppedUrl || element.url"
							...
							/>
							<UiButton
								class="gallery-load-secondary__btn gallery-load-secondary__btn_type_delete"
								type="button"
								variant="default"
								:without-padding="true"
								@click="handleDeleteImage(element.id)"
							>
							...
							<UiNewDropdown
								...
								placement="top"
								:arrow="true"
								class="gallery-load-secondary__btn gallery-load-secondary__btn_type_setting"
								:offset="[100, 20]"
							>
								<UiButton
									class="gallery-load-secondary__tippy-btn"
									type="button"
									variant="default"
									:without-padding="true"
								>
								...
								<template #content>
									<div class="gallery-load-secondary__tippy-container">
										...
										<UiButton
											class="gallery-load-secondary__tippy-item"
											type="button"
											variant="quinary"
											size="large"
											@click="handleMovingForGroup(element)"
										>
											{{ movingText }}
										</UiButton>
									</div>
		...
		>
			...
			<UiButton
				class="gallery-load-secondary__btn-resize"
				type="button"
				variant="quinary"
				size="large"
				@click="handleCrop(imagePreview)"
			>
				Выбрать область
			</UiButton>
		</ModalsRoundBorder>
...
<script setup>
	...
	const props = defineProps({
		label: {
			type: String,
			default:
				'Загрузить фотографию (до 5Мб. Допустимый формат .jpeg, .png, .jpg, .gif)',
		},
		modelValue: {
			type: Array,
			default: () => [],
	...
	const isUploadDisabled = computed(
		() =>
			props.maxCount !== null && localModelValue.value.length >= props.maxCount
	);
	...
	const onFileChange = (event) => {
		if (props.maxCount !== null && props.modelValue.length >= props.maxCount) {
			toast.error(`Можно загрузить не более ${props.maxCount} изображений`);
			return;
		}
...
<style lang="scss">
	...
	.gallery-load-secondary__item {
		transition:
			transform 0.3s cubic-bezier(0.65, -0.04, 0.41, 0.77),
			box-shadow 0.3s cubic-bezier(0.65, -0.04, 0.41, 0.77);
	}
</style>
