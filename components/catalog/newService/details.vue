	...
	<div class="new-service-details">
		...
		>
			...
			<div class="new-service-details__pub-card-content">
				<div class="new-service-details__pub-card-top">
					<h3 class="new-service-details__pub-card-title">
						{{ data.name || 'не указано' }}
					</h3>
					<CommonUserOnlineStatus
						class="new-service-details__pub-card-user-status"
						:lastActivity="data.last_active_at"
					/>
				</div>
				<CommonRating
					:rating="data.reviews_stats_about?.stars"
					:reviews="data.reviews_about_count"
				...
				/>
				<div
					class="new-service-details__pub-card-site"
					v-if="data?.url_site"
				>
					<SvgoPlanet class="svg-m" />
					<NuxtLink
						:to="ensureHttps(data.url_site)"
						class="link"
						target="_blank"
					>
						{{ data.url_site || 'не указано' }}
					</NuxtLink>
				</div>
				<!-- КНОПКИ ДЛЯ ДЕСКТОПА -->
				<div
					class="new-service-details__pub-card-buttons"
				...
				>
					...
					<UiButton
						...
						class="new-service-details__pub-card-button"
						:class="{
							'new-service-details__pub-card-button_type_active': isFavorite,
						}"
		...
		>
			...
			<div class="new-service-details__pub-card-content">
				...
				<CommonUserOnlineStatus
					class="new-service-details__pub-card-user-status new-service-details__pub-card-user-status_type_mobile"
					:lastActivity="data.last_active_at"
				/>
				<!-- КНОПКИ ДЛЯ МОБИЛКИ -->
				<div
		...
		<div class="new-service-details__specs">
			...
			<CatalogNewServiceDetailsBadge
				:specs="{
					...
					value: data.free_samples?.length
						? entityStore
								.getEntityLabelById('freeTestSamples', data.free_samples)
								.join(' / ')
						: '',
				}"
			/>
			<div
				class="new-service-details__specs-contacts"
				v-if="contactsData"
			>
				<CatalogNewServiceDetailsBadge
					:specs="{
			...
			<CatalogNewServiceContactsButton
				:id="data.id"
				v-if="data.is_open_contacts_active && !contactsData"
				v-slot="{ open }"
				@show="showContacts"
			>
		...
		<CommonLayoutInfoCard
			title="Услуги"
			class="new-service-details__services"
			v-if="data.services && data.services.length"
		>
			<div class="new-service-details__services-list">
				<div
					class="new-service-details__services-item"
					v-for="(item, index) in data.services"
					:key="item.id"
				>
					<div class="new-service-details__services-item-number">
						<span>{{ index + 1 }}</span>
					</div>
					<div class="new-service-details__services-item-content">
						<h4 class="new-service-details__services-item-name">
							{{ item.name || 'не указано' }}
						</h4>
						<p
							class="new-service-details__services-item-prop"
							v-if="item.batch"
						>
							Размер партии:
						...
						<div
							class="new-service-details__services-item-badges"
							v-if="item.product_categories"
						>
							<div
								class="new-service-details__services-item-badge"
								v-for="category in item.product_categories"
								:key="category.id"
							>
								<p>{{ category.name }}</p>
							</div>
		...
		<CommonLayoutInfoCard
			title="Описание"
			class="new-service-details__description"
		>
			<p class="new-service-details__description-text">
				{{ data.description || 'не указано' }}
			</p>
		</CommonLayoutInfoCard>
		<CommonLayoutInfoCard
			title="Примеры работ"
			class="new-service-details__gallery"
		...
		>
			...
			/>
			<template #action>
				<p class="new-service-details__gallery-count">
		...
		<CommonLayoutInfoCard
			...
			class="new-service-details__description"
			v-if="data.equipment && data.equipment.length"
		>
			<p class="new-service-details__description-text">
				{{ data.equipment_description || 'не указано' }}
			</p>
		</CommonLayoutInfoCard>
		...
		>
			...
			<template #action>
				<p class="new-service-details__gallery-count">
					{{ `${currentEquipmentIndex + 1}/${data.equipment.length}` }}
				</p>
			</template>
		</CommonLayoutInfoCard>
		<CommonLayoutInfoCard
			title="Отзывы"
			class="new-service-details__reviews"
			v-if="reviewList.length"
		>
			<CatalogNewServiceReviewList
				:reviewList="reviewList"
				:pub_card="data"
			/>
			<CommonPagination
				v-if="reviewsPage.last_page > 1"
				:current-page="reviewsPage.page"
				:total-pages="reviewsPage.last_page"
				@change-page="handleChangeReviewsPage"
				btn-type="square"
				position="left"
			/>
		</CommonLayoutInfoCard>
...
<script setup>
	...
	const fetchReviews = async (page = 1) => {
		await nextTick();
		reviewStore
			.getOrganizationReviews(props.data.organization_id, { page })
			.then((res) => {
	...
	function handleAddFavorite() {
		...
		if (isFavorite.value) {
			userStore
				.removeFavorite(userStore.userData.id, props.data.id, 'pubCard')
				.then((res) => {
			...
			userStore
				.addFavorite(userStore.userData.id, props.data.id, 'pubCard')
				.then((res) => {
