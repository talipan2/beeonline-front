		...
		<template #bottom>
			<nav class="sidebar__bottom">
				<template v-if="userStore.role !== 'adjacent'">
					<button
						v-if="
							...
							userData.roles?.includes('performer')
						"
						@click="handleChangeRole('performer')"
					...
					<button
						...
						"
						@click="handleChangeRole('customer')"
						class="sidebar__bottom-link"
					>
						<SvgoAdduser
							class="svg-m"
					...
					<button
						...
						v-if="!userRoles.includes('performer')"
				...
				<NuxtLink
					...
					:to="`/support`"
					:class="{
						'sidebar__bottom-link_type_active': isActiveLink('/support'),
					}"
				>
					<SvgoSupport
						class="svg-m"
...
<script setup>
	...
	const handleChangeRole = async () => {
		...
		try {
			await userStore
				.setUserData({ role: newRole }, userStore.userData.id)
				.then((res) => {
	...
	const isActiveLink = (link) => {
		if (link.includes('/reviews') || link.includes('/my-reviews')) {
			return (
				router.currentRoute.value.path.includes(link) ||
				router.currentRoute.value.path.includes('/reviews')
			);
		}
