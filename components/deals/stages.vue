<template>
  <div class="deals-stages">
    ...
    <p class="deals-stages__text">
      Исполнитель предлагает сотрудничество и даёт возможность отслеживать
      работу над вашим заказом. Команда Bee-online подготовила новый инструмент,
      который позволит вам безопасно и эффективно взаимодействовать с
      производителями по всей стране. Для удобства процесс разбит на несколько
      этапов. Вы можете отслеживать здесь этапы работы над заказом.
    </p>
    <div class="deals-stages__stages">
      <template v-for="(step, index) in steps" :key="index">
        <div
          class="deals-stages__stage"
          :class="{ 'deals-stages__stage_type_active': step.active,
          'deals-stages__stage_type_completed': step.status === 'completed' }"
        >
          <div
            class="deals-stages__header"
            @click="step.isOpenContent = !step.isOpenContent"
          >
            <div class="deals-stages__number" v-if="step.stage === 'arbitrage'">
              <span>×</span>
            </div>
            <div class="deals-stages__number" v-else >{{ index + 1 }}</div>
            <h3 class="deals-stages__header-title">{{ step.title }}</h3>
            <UiButton
              type="button"
              class="deals-stages__btn"
              variant="default"
              size="around"
              :class="{ 'deals-stages__btn_type_active': step.isOpenContent }"
            >
              <SvgoDropDownNew class="svg-m" />
            </UiButton>
          </div>
          <div class="deals-stages__body" v-if="step.isOpenContent">
            <component
              :is="step.content"
              v-if="
...
const arbitrage = true

watch(() => steps.value, () => {
  let shouldBeActive = false;
  if (arbitrage) {
    const activeIndex = steps.value.findIndex((step) => step.active);

    if (activeIndex >= 0 && steps.value[activeIndex].stage !== "arbitrage") {
      ...
      const exists = steps.value.some((step) => step.stage === "arbitrage");
      if (!exists) {
