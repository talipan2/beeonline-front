<template>
	<div class="entity">
		...
		>
				...
				<div class="entity__data-group">
					<div class="entity__data-item">
						...
						<UiCheckboxGroup
							...
							name="rawMaterials"
							label="Сырье"
							class="entity__group form-group__value"
			...
			<div
				class="entity__data entity__data_type_customer"
				v-else
			>
				<div class="entity__data-group">
					<div class="entity__data-item">
					...
					<div class="entity__data-item">
						<label class="form-group__title entity__label">
							Объем партии *
							<CommonTooltip
								text="Укажите количество в единицах измерения - шт."
							/>
						</label>
						<UiInput
          ...
          - doc, .docx, .xls, .xlsx, .ppt, .pptx, .rtf, .pdf, .jpeg, .png, .jpg, .gif, .psd, .djvu, .fb2, .ps, .zip, .rar"
			...
			<CommonAlerts
				v-if="errorList.selectedLocations"
				:alert="errorList.selectedLocations"
			...
			<div
				...
				v-if="role === 'customer'"
			>
				<h2 class="entity__subtitle">
					Регионы фактического производства заказа *
				</h2>
				<div class="entity__text-container">
					<p
						...
						v-if="role === 'performer'"
					>
						Укажите город вашего производства. Если производств несколько -
						выберите несколько городов, но не более пяти.
					</p>
					<p
						class="entity__text"
						v-else
					...
					<p
						...
						v-if="role === 'performer'"
					>
						Потенциальный заказчик сможет вас найти по регионам указанных
						городов.
					</p>
...
<script setup>
	...
	const freeStockOptions = computed(
		() => entityStore.entityData?.freeStock || []
	);
