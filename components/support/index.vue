	...
	<div class="support">
		<div class="support__header">
			<p class="support__text">
				Вы можете сформировать запрос в нашу службу поддержки или отправить
				запрос на почту
				<a
					href="mailto:service@bee-online.ru"
					class="link"
				>
					service@bee-online.ru.
				</a>
			</p>
			<UiButton
				to="/support/create"
		...
		<div class="support__list">
			...
			<div
				class="support__list-item-container"
			...
			>
				<div class="support__list-item support__list-item_type_solved">
					...
					<p class="support__list-col-2">
						{{
							formatDate(ticket.updated_at, 'weekday, DD.MM.YYYY - HH:mm:ss')
						}}
					</p>
					<p class="support__list-col-3">{{ ticket.status_name }}</p>
					<p class="support__list-col-4">{{ ticket.type_name }}</p>
					<NuxtLink
						class="support__list-link"
...
<script setup>
	...
	const supportStatuses = computed(() => {
		const list = [{ id: 0, label: 'Все', value: 0, disabled: false }];
		return list.concat(
			supportStore.getSupportTicketStatuses.map((item) => ({
				id: item.id,
				label: item.label,
				value: item.value,
			}))
