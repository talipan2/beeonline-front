		...
		>
			...
			<label class="support-create__label form-group__title">
				...
				<CommonDropDownSelect
					class="form-group__value support-create__select"
					:options="typeList"
					v-model="appealData.type"
					name="type"
					label="Тема обращения"
...
<script setup>
	...
	const typeList = computed(() => {
		...
		return list.concat(
			supportStore.getSupportTicketTypes.map((item) => ({
				id: item.id,
				label: item.label,
				value: item.value,
			}))
	...
	const handleSubmit = (values, form) => {
		...
		return supportStore.createSupportTicket(data).then((response) => {
			form.resetForm();
			appealData.value = {
				type: null,
				situation: '',
