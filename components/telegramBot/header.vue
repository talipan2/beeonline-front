    ...
    <div class="header__main" ref="headerMain" >
      <div class="header__main-container" :class="{ 'container': headerFixed }">
        <div class="header__logo-container">
          <button class="header__menu-btn" :class="{ 'header__menu-btn_active': isOpenMobileModal }" @click="isOpenMobileModal = !isOpenMobileModal" v-if="isAuth">
            <div></div>
            <div></div>
            <div></div>
          </button>
...
//   userStore.role = localStorage.getItem('role');
  userStore.checkAuth().then(() => {
    if(userStore.userData.id) {
      router.push({ path: `/customer/orders`, query: { platform: 'telegram' } });
    }
  });
