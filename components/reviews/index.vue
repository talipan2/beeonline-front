  ...
  />
		...
		>
			...
			<span class="reviews__rating-value">
				{{ pubCardRate.average_rating }}/5
			</span>
			<CommonRating
				:is-count-rating="false"
				:is-count-reviews="false"
				:rating="pubCardRate.stars"
			/>
		</div>
		...
		<div class="reviews__list">
			<template v-if="currentButtonType === 'my-reviews'">
				<template
					v-for="review in reviewsListFormatted"
					:key="review.id"
				>
					<ReviewsCard
						:data="review"
						:reviewsState="currentButtonType"
					/>
				</template>
				<CommonAlerts
					:alert="alertEmptyText"
					:type="'warning'"
					v-if="!reviewsListFormatted.length && isLoaded"
				/>
				<CommonPagination
			...
			<template v-if="currentButtonType === 'reviews'">
				...
				<CommonAlerts
					:alert="alertEmptyText"
					:type="'warning'"
					v-if="!reviewsAboutUsListFormatted.length && isLoaded"
				/>
				<CommonPagination
...
<script setup>
	...
	// Маппинг имен фильтров для бэкенда
	const filterMapping = {
		rate: 'rate', // рейтинг остается как есть
		org_type: 'org_type', // тип организации остается как есть
		reviews_status: 'status', // статус отзыва на бэкенде называется 'status'
	};
	...
	const filterList = computed(() => {
		if (
			currentButtonType.value === 'my-reviews' &&
			countRejectReviews.value > 0
		) {
			return ['rate', 'org_type', 'reviews_status'];
		} else if (
			currentButtonType.value === 'my-reviews' &&
			countRejectReviews.value === 0
		) {
			return ['rate', 'org_type'];
		} else {
			return ['rate', 'org_type'];
		}
	});
	...
	function handleChangePage(page) {
		switch (currentButtonType.value) {
			case 'my-reviews':
				const queryMyReview = deleteEmptyFilters({
					...activeFilterMyReviews.value,
				});
				...
				break;
			case 'reviews':
				const queryReviewsAboutUs = deleteEmptyFilters({
					...activeFilterReviewsAboutUs.value,
				});
	...
	const getReviews = (filters) => {
		...
		reviewStore
			.getReviews(organizationId.value, filters)
			.then((res) => {
				if (res) {
					if (res.data) {
				...
				router.replace({
					query: {
						...
						page: pageReviews.value.currentPage,
						...activeFilterMyReviews.value,
					},
				});
			})
	...
	const getReviewsForUs = (filters) => {
		...
		isLoading.value = true;
		reviewStore
			.getReviewsForUs(organizationId.value, filters)
			.then((res) => {
				if (res) {
					if (res.data) {
				...
				router.replace({
					query: {
						...router.currentRoute.value.query,
						page: pageReviewsAboutUs.value.currentPage,
						...activeFilterReviewsAboutUs.value,
					},
				});
			})
	...
	if (currentButtonType.value === 'my-reviews') {
		...
		reviewsAboutUsParams = deleteEmptyFilters({
			...activeFilterReviewsAboutUs.value,
		});
		if (query.page) {
	...
	isLoading.value = true;
	await reviewStore
		.getReviews(organizationId.value, reviewsParams)
		.then(async (res) => {
			if (res) {
				if (res.data) {
					reviewsList.value = res.data;
					...
					await reviewStore
						.getCountRejectReviews(userStore.userData?.organization_id)
						.then((res) => {
							if (res) {
								countRejectReviews.value = res?.count || 0;
							}
						});
				}
			}
	...
	await reviewStore
		.getReviewsForUs(organizationId.value, reviewsAboutUsParams)
		.then((res) => {
			if (res) {
				if (res.data) {
					reviewsAboutUsList.value = res.data;
