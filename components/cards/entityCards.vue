<template>
	<div
		class="entity-card"
		:class="{ 'entity-card_hover': isLinkHovered }"
	...
	>
		...
		<div class="entity-card__body">
			<div
				class="entity-card__details"
				v-if="data.placeOfProduction && data.placeOfProduction.length > 0"
			>
				<div class="entity-card__region">
					...
					<ModalsMoreCities
						...
						v-if="
							Array.isArray(data.placeOfProduction) &&
							data.placeOfProduction.length >= 1
						"
					/>
				</div>
			...
			>
				...
				<p class="entity-card__details-value">
					{{ Number(data.batch) || 'Не указано' }}
				</p>
			</div>
			<div
				class="entity-card__details"
			...
			>
				...
				<p class="entity-card__details-value">
					{{
						data.minLot && data.minLot.length ? data.minLot[0] : 'Не указано'
					}}
				</p>
			</div>
			<div
			...
			>
				...
				<p class="entity-card__details-value">
					До {{ formatDate(data.completionDate) || 'Не указано' }}
				</p>
			</div>
		</div>
		<div class="props">
			<div class="prop">
				...
				<p class="prop__value">
					{{ data.rawMaterials.join(' / ') || 'Не указано' }}
				</p>
			</div>
			<div class="prop">
		...
		<div class="entity-card__footer">
			...
			<template v-if="data.statusType === 'active'">
				...
				<template v-if="type === 'service'">
					<PaidServiceRaisingService :id="data.id">
				...
				>
					<template #button="{ open }">
						...
						>
							...
							<template v-else>Добавить ссылку на форму сбора</template>
						</UiButton>
					</template>
				</PaidServiceOrderIntro>
			</template>
		...
		<NuxtLink
			...
			:to="`/performer/services/show/${data.id}`"
			v-if="role === 'performer'"
		></NuxtLink>
		<NuxtLink
			class="entity-card__link"
			:to="`/customer/orders/show/${data.id}`"
			v-else
		></NuxtLink>
		<CatalogOrdersQuestionnaire
