<template>
	...
	<div class="form-group form-group_type_secondary">
		<label class="form-group__title">
			Контактное лицо (ФИО) (Будет доступно всем пользователям сайта при покупке
			услуги "Открытые контакты")
		</label>
		<UiInput
	...
	<div class="form-group form-group_type_secondary">
		<label class="form-group__title">
			Контактный email (Будет доступно всем пользователям сайта при покупке
			услуги "Открытые контакты")
		</label>
		<UiInput
			:rules="{ required: false }"
	...
	<div class="form-group form-group_type_secondary">
		<label class="form-group__title">
			Контактный телефон (Будет доступно всем пользователям сайта при покупке
			услуги "Открытые контакты")
		</label>
		<UiInput
			:rules="{ required: false }"
	...
	<CommonAlerts
		v-if="errorsList.selectedLocations && !skipLocationError"
		:alert="errorsList.selectedLocations"
		alertType="validation"
	/>
	<div class="form-group form-group_type_secondary">
		<label class="form-group__title">Выбрать город</label>
		<p class="form-group-description">
			Укажите местонахождение вашего производства, в случае если у вас несколько
			фабрик - укажите все локации производства, не более 5.
		</p>
		<CommonLocation
	...
	<div class="form-group form-group_type_secondary">
		<label class="form-group__title">Количество швей</label>
		<UiInput
			:rules="{ required: true }"
			name="workers_count"
	...
	<div class="form-group form-group_type_secondary pub-card-form-checkbox">
		<UiCheckbox
			:rules="{ required: true }"
			v-model="modelValue.privacy_policy"
			name="privacy_policy"
